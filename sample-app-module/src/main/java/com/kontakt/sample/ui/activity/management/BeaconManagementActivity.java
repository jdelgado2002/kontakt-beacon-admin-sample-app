...
public class BeaconManagementActivity extends BaseActivity implements IBeaconConnection.ConnectionListener {
    ...
    private void onConfigResultDelivered(final int resultCode, final Intent data) {
        if(resultCode != RESULT_CANCELED) {
            final ConfigIBeacon config = data.getParcelableExtra(ConfigFormActivity.EXTRA_RESULT_CONFIG);
    ...
    private void onApplyConfig(final ConfigIBeacon config) {
        beaconConnection.applyConfig(config, new IBeaconConnection.WriteBatchListener<ConfigIBeacon>() {
            @Override
            public void onWriteBatchStart(ConfigIBeacon batchHolder) {
            ...
            @Override
            public void onWriteBatchFinish(final ConfigIBeacon batch) {
